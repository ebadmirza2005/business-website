...
const postForm = async (url, payload) => {
  ...
  if (!response.ok || !data.success) {
    ...
    if (data.error) messageParts.push(String(data.error));
...
if (contactForm) {
  contactForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const result = await postForm("send_request.php", { name, email, service, message });
...
const initPackageTabs = () => {
  ...
  const packageGridWrappers = document.querySelectorAll(
  ...
  packageTabButtons.forEach((tabButton) => {
    tabButton.addEventListener("click", () => {
      ...
      packageTabButtons.forEach((button) =>
        button.classList.remove("is-active"),
      );
      packageGridWrappers.forEach((wrapper) =>
